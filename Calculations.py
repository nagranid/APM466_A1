...
def combine():
    ...
    Prices= get_prices(20220110,20220121,"x")
    i=0
    while i<len(Prices):
...
def Yield_To_Maturity(No_of_Bonds):
    ...
    while i<len(Bonds):
        ...
        count=0
        while count<len(Dates):
            CF_schedule=CF[i][Dates[count]]
            Timing=[]
            j=0
            while j<len(CF_schedule):
...
def Spot_Rate(No_of_Bonds):
    ...
    for x in dic:
        ...
        t=float(t / timedelta (days=1))
        s1=dt.strptime(x, '%Y-%m-%d').date()
        e1=dt.strptime("9/1/2023", '%m/%d/%Y').date()
        t1=(e1-s1)/365
        t1=float(t1 / timedelta (days=1))
    ...
    for x in dic:
        ...
        first=(coup1)*math.e**(-r1*t)
        second=(coup2)*math.e**(-r2*(t+.5))
        third=(coup3)*math.e**(-r3*(t+1))
        fourth=(coup4)*math.e**(-r4*(t+1.5))
    ...
    for x in dic:
        ...
        t=float(t / timedelta (days=1))
        s1=dt.strptime(x, '%Y-%m-%d').date()
        e1=dt.strptime("9/1/2024", '%m/%d/%Y').date()
        t1=(e1-s1)/365
        t1=float(t1 / timedelta (days=1))
    ...
    for x in dic:
        ...
        third=(coup3)*math.e**(-r3*(t+1))
        fourth=(coup4)*math.e**(-r4*(t+1.5))
        fifth=(coup5)*math.e**(-r5*(t+2))
        sixth=(coup6)*math.e**(-r6*(t+2.5))
    ...
    for x in dic:
        ...
        t=float(t / timedelta (days=1))
        s1=dt.strptime(x, '%Y-%m-%d').date()
        e1=dt.strptime("9/1/2025", '%m/%d/%Y').date()
        t1=(e1-s1)/365
        t1=float(t1 / timedelta (days=1))
    ...
    for x in dic:
        ...
        fifth=(coup5)*math.e**(-r5*(t+2))
        sixth=(coup6)*math.e**(-r6*(t+2.5))
        seventh=(coup7)*math.e**(-r7*(t+3))
        eigth=(coup8)*math.e**(-r8*(t+3.5))
    ...
    for x in dic:
        ...
        seventh=(coup7)*math.e**(-r7*(t+3))
        eigth=(coup8)*math.e**(-r8*(t+3.5))
        ninth=(coup9)*math.e**(-r9*(t+4))
        tenth=(coup10)*math.e**(-r10*(t+4.5))
